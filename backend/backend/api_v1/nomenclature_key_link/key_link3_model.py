...
class NomenclatureKeyLink3(LegacyCreatedAtMixin, Base):
    __tablename__ = "nomenclaturekeylink3"

    id: Mapped[int] = mapped_column("Id", Integer, primary_key=True)
    status_id: Mapped[int] = mapped_column("IdStatus", ForeignKey("nomkeylinkstatustype.Id"))
    parent_id: Mapped[int] = mapped_column("IdParent", ForeignKey("nomenclaturekeylink2.Id", ondelete="CASCADE"))
    key_id: Mapped[int] = mapped_column("IdKey", ForeignKey("nomenclaturekey.Id"))

    status = relationship("NomKeyLinkStatusType", lazy="joined")
    parent = relationship("NomenclatureKeyLink2", back_populates="children", lazy="joined")
