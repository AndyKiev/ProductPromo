"""ProductPromo knowledge-vault CLI (Obsidian wiki loop).
...
def summarize(text):
    ...
    for line in body_of(text).splitlines():
        ...
        if re.search(r"[^\W\d_]{4}\.(\s|$)", line):
            break
    if not collected:
        return ""
    sentence = " ".join(collected)
    sentence = re.sub(r"[*`\[\]]", "", sentence)
    # A sentence ends at ". " before a capital, after a word of 4+ letters -
    # so "excl. VAT" and a trailing "1." list marker don't cut it short.
    match = re.search(r"(?<=[^\W\d_]{4})\.\s+(?=[A-ZА-ЯІЇЄ])", sentence)
    head = sentence[:match.start() + 1] if match else sentence
    return head[:160]
