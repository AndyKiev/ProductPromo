...
It is outside the project, so it must stay listed in `.Codex/settings.local.json`
...
| **Derived** | `Architecture/`, `Coding-Standards/`, `API/`, plus `index.md`, `hot.md` | `/wiki-ingest` from repo sources (`AGENTS.md`, `AGENTS.md`, `.Codex/skills/*`, code) | **No** — re-ingest instead; hand edits are lost |
...
is what closes the loop. The snippet for `.Codex/settings.local.json`:
